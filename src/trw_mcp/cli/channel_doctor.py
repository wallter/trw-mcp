...
Provides five sub-commands (argparse dispatched from _subcommands.py):
  ...
  trw-mcp channel-doctor stats     — show correlation stats table

The `throttle` sub-command was removed 2026-09-22 (RC-014): it rewrote
...
# ---------------------------------------------------------------------------
# stats sub-command (meta-tune consumer)
# ---------------------------------------------------------------------------
...
def run_channel_doctor(args: argparse.Namespace) -> None:
    ...
    if sub == "init":
        ...
        print(
            "Usage: trw-mcp channel-doctor {init,validate,scan,clean,stats} [options]\n"
            ...
            "  stats     Show per-channel correlation stats\n"
        )
