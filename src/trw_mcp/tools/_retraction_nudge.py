...
(``trw_learn(learning_id=..., metadata={"supersedes": ...})``); nothing ever asks anyone to use it, so
...
__all__ = ["unretracted_contradiction_nudge", "unsettled_contradiction_ids"]
...
def unretracted_contradiction_nudge(trw_dir: Path) -> str:
    ...
    try:
        ...
        calls = "; ".join(
            f"trw_learn(learning_id='{lid}', status='obsolete')  # or metadata={{'supersedes': <new id>}}"
            for lid in named
        )
    ...
    except Exception:  # justified: fail-open, an advisory must never block delivery  # trw-fail-silent-allow: an advisory nudge must never block delivery; only its call text changed (PRD-CORE-291)
