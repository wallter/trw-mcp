...
- _ceremony_maintenance_steps: version sentinel, WAL checkpoint, learn-journal
  drain (the fail-open auto-maintenance sub-steps)
"""
...
def run_auto_maintenance(
    ...
    # WAL checkpoint against the checkout's own memory.db was retired here:
    # PRD-CORE-298 moved every checkout onto the daemon store, which owns and
    # checkpoints its own WAL. Opening the checkout db from this path re-reads
    # a file the daemon may hold open and that this process should never touch.

    _run_learn_journal_drain(trw_dir, config, maintenance)
