"""Structured-argument bags for ``trw_learn``'s create and update modes.
...
``trw_learn`` was the largest signature in the package, and its update mode
(once a separate update tool) the second; the rarely-set fields of each mode
now travel in the one ``metadata`` object, validated against that mode's model.
...
class LearnMetadata(BaseModel):
    """Type-normalized ``trw_learn(metadata=...)`` values when creating a learning.
...
class LearnUpdateFields(BaseModel):
    """Type-normalized ``trw_learn(learning_id=..., metadata=...)`` values when updating.
