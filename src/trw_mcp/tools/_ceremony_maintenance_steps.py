...
# One daemon thread at a time, cleared in a ``finally`` only while it still points at the
...
def _finish_drain_sweep(
    ...
    * a remainder the running worker will pick up is reported as
    ...
    # FR02: only a BUDGET stop earns a continuation. A count-limit deferral
    # keeps its current meaning — those records wait for the next sweep and
    # are still reported via pending_learns_deferred.
    remainder = min(deferred, max(0, limit - replayed_inline)) if budget_exhausted else 0
    scheduled = bool(remainder) and _schedule_background_drain(trw_dir, config, remainder, sweep=sweep)
    if not scheduled:
        sweep.flush()
...
def _schedule_background_drain(
    ...
    *,
    ...
    Single-flight: a second overlapping call starts no second thread and says so. Returns True
    ...
    **Daemon durability.** ``daemon=True`` means interpreter exit can kill this
    ...
    def _run_drain() -> None:
        ...
        try:
            run_background_sweep(trw_dir, config, background_limit, sweep)
        finally:
            with _DRAIN_LOCK:
