...
def format_markdown(audit: dict[str, object]) -> str:
    ...
    lines: list[str] = []
    lines.append(f"# TRW Audit Report — {audit.get('project', 'unknown')}")
    lines.append(f"Generated: {audit.get('generated_at', '')}")
    lines.append("")

    # Learnings section
    learnings = audit.get("learnings", {})
    ...
    # Index consistency
    idx = audit.get("index_consistency", {})
    ...
    recall = audit.get("recall_effectiveness", {})
    if isinstance(recall, dict) and recall.get("verdict") != "SKIP":
    ...
    # Ceremony compliance
    ceremony = audit.get("ceremony_compliance", {})
    ...
    # Reflection quality
    reflection = audit.get("reflection_quality", {})
    ...
    # Hook versions
    hooks = audit.get("hook_versions", {})
    if isinstance(hooks, dict) and hooks.get("verdict") != "SKIP":
