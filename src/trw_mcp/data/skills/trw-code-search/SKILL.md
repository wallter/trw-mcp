...
1. Refresh the local manifest with `trw-mcp code index <repo_root>` (add `--force`
   to reclassify every file, or repeat `--paths <path>` to limit the build).
2. Search behavior with `trw_code(mode="search", repo_root=..., query=..., top_k=10)`.
3. Locate declarations with `trw_code(mode="symbol", repo_root=..., query=<symbol>, top_k=10)`.
...
- Lexical search works in the base install and does not require parser or
  embedding extras.
- Search is lexical. `trw_code` has no semantic search mode and no embedding
  extra to install; its `mode` takes only `search`, `symbol` or `hint`.
...
1. Round trip: after `trw-mcp code index`, a symbol you can see in a file you
   have open must come back from `trw_code(mode="symbol", ...)` with that file
   and a line range that actually contains it.
2. Stale index: edit an indexed file without re-running
   `trw-mcp code index`, and confirm the hit you get back still points at a
