...
5. Record any durable gotchas with `trw_learn(...)`. For a recalled or hinted learning that changed a decision or saved work, call `trw_learn_update(learning_id="<id>", feedback="helpful")`; for one that misled you, call `trw_learn_update(learning_id="<id>", feedback="unhelpful")`. Do neither for a learning that was only shown: exposure alone is not feedback.
