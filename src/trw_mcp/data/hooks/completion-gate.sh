#!/bin/sh
# PRD-FIX-024 / PRD-INFRA-004-FR01: CompletionGate compatibility hook — conditional quality gate.
# Logs completion events. Blocks (exit 2) delegated helpers who
# haven't run trw_build_check (FR01) or haven't checkpointed (FR03).
# TRW_SOFT_GATES=1 disables all blocking (FR04).
# Helper completions are never blocked (they are intermediate steps).
...
# FR01: Hard gate — check build status for helper completions
...
if [ "$_tests_passed" != "true" ]; then
  ...
  log_hook_execution "CompletionGate" "$_helper_name:${_task_subject}" "2:build-fail"
  trap - EXIT
  exit 2
fi

# --- Helper context: conditional hard gate (checkpoint check, FR03) ---
_context_dir="$_project_root/.trw/context"
[ -d "$_context_dir" ] || mkdir -p "$_context_dir" 2>/dev/null || exit 0

# Block count file per (helper, task) to limit to 1 block each
...
if [ "$_has_checkpoint" -eq 1 ]; then
  # Helper has checkpointed — now check for completion artifact
  _completion_artifact=""
  if [ -n "$_run_dir" ]; then
    _scratch_dir="${_run_dir}scratch/helper-${_safe_name}/completions"
  ...
  if [ -n "$_completion_artifact" ]; then
    ...
    log_hook_execution "CompletionGate" "${_helper_name:-unknown}:${_task_subject}" "0:artifact-verified"
    exit 0
  fi
  ...
  if [ "$_blocks" -ge 2 ]; then
    ...
    log_hook_execution "CompletionGate" "${_helper_name:-unknown}:${_task_subject}" "0:escape-valve"
    exit 0
  fi

  _blocks=$((_blocks + 1))
  printf '%s' "$_blocks" > "$_block_file" 2>/dev/null || true
  echo "TRW: Before completing, write a completion artifact to scratch/helper-${_safe_name}/completions/ with:" >&2
  ...
  log_hook_execution "CompletionGate" "${_helper_name:-unknown}:${_task_subject}" "2:needs-artifact"

  _trw_intentional_exit=1
  exit 2
fi

# Block: ask helper to checkpoint before completing
_blocks=$((_blocks + 1))
printf '%s' "$_blocks" > "$_block_file" 2>/dev/null || true
echo "TRW: Running trw_checkpoint preserves your task results for the orchestrator. Call trw_checkpoint(message) with a summary of what you completed and which FRs you addressed. (Block $_blocks/2)" >&2

log_hook_execution "CompletionGate" "${_helper_name:-unknown}:${_task_subject}" "2"
