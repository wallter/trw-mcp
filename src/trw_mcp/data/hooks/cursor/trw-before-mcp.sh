#!/usr/bin/env bash
# TRW Cursor hook — beforeMCPExecution
# Observer: logs MCP tool call, emits allow permission.
set -euo pipefail
...
_log() {
  ...
  printf '{"ts":"%s","level":"info","component":"cursor-hook","event":"beforeMCPExecution","msg":%s}\n' \
...
_log "beforeMCPExecution tool=${_TOOL}"
