...
| standard | + events.jsonl, build records | 10 | ≤ 2 (read-only) |
...
At `standard`+ depth, you MAY delegate (2) and (6) to a read-only helper agent
if your client supports sub-agents; otherwise collect them inline. Keep the
...
- Interactive session: present the table in chat and ask for per-row approval
  (approve all / approve subset by number / reject).
