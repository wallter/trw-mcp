#!/bin/sh
# GM-01 shared helper library for the Gemini BeforeTool distill hint hook.
#
# PRD-DIST-2459 FR-3. Mirrors the Claude Code lib-distill-hint.sh contract
# (config gate, python-path resolution, safe-extension allowlist) so the
# Gemini hook reuses the SAME activation gate (cc03_hook_enabled), the SAME
# sidecar contract (read via compute_before_edit_hint), and the SAME
# skip-extensions allowlist — no per-profile fork (FR-6).
#
# This is a sibling of the Claude lib (NOT a copy imported by path) so the
# Gemini install ships standalone. POSIX sh only — no bashisms.
#
# Provides:
...
_get_cc03_enabled() {
    ...
    #
    # Precedence (matches Claude lib + Python read_cc03_config):
