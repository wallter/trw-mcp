...
_PROFILE_DIR_SURFACES: dict[str, tuple[UninstallSurface, ...]] = {
    "claude-code": (
        ...
        # merges TRW hook entries into an existing one. Every bundled hook
        # command points into ``.claude/hooks/``, which uninstall deletes
        # wholesale for claude-code — so leaving the entries registered makes
        # every later Claude Code session invoke a script that no longer
        # exists. It is the user's client config (permissions, env, their own
        # hooks), so it is stripped, never deleted. This file, and every other
        # surface in this tuple, is claude-code's ONLY — moved out of
        # ``_CORE_SURFACES`` (C7, PRD-INFRA-192 FR09) where they were written
        # and removed for every client, including ones that never read
        # ``.claude/`` at all (cursor, opencode, grok, ...).
        ...
        # user-configured servers", so deleting the file wholesale destroys
        # every unrelated server the user configured (github, postgres, ...).
        # Strip only the ``trw`` entry; the file itself is always preserved (a
        ...
        # run scripts from here (bootstrap/_codex_hooks.py, bootstrap/
        # _copilot.py), so it is declared again under those two client
        # entries below and uninstall only removes it when no OTHER recorded
        # client still owns it (``_surfaces_declared_by_others``).
    ...
    "codex": (
        ...
        UninstallSurface(".codex/hooks.json", merged_config=True, config_shape="codex-hook-group-list"),
        UninstallSurface(".codex/hooks"),
        # SHARED with claude-code (declared there too): codex's own hook
        # commands run ``.claude/hooks/<script>`` (bootstrap/_codex_hooks.py),
        # so an explicit codex-only install still needs this directory. See
        # the claude-code entry above for the uninstall-ownership note.
        UninstallSurface(".claude/hooks"),
    ),
    "copilot": (
        # SHARED with claude-code (declared there too): copilot's own hook
        # commands also run ``.claude/hooks/<script>`` (bootstrap/_copilot.py).
        UninstallSurface(".claude/hooks"),
        ...
        UninstallSurface(".github/hooks/hooks.json", merged_config=True, config_shape="copilot-hook-group-list"),
