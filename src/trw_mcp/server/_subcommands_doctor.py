...
#
# Shared with uninstall (imported at the top of this module) rather than
# hand-copied. This table previously carried three pairs against the eleven TRW
# actually writes, so `doctor` could not see a distill, memory, codex,
# cursor-MDC, git-shim or uppercase-AGENTS block — it reported those surfaces as
# carrying no TRW section, which is indistinguishable from a clean project.
# Uninstall had the identical 3-pair copy and the identical blindness; the two
# are now one derivation, so the next channel that adds a marker is covered here
# for free.
...
def _check_instruction_carrier_state(target: Path, _config: TRWConfig) -> CheckResult:
    ...
    if not states:
        return CheckResult("instruction_carrier", "SKIP", "no instruction surface present yet.")
    ...
    return CheckResult("instruction_carrier", "PASS", detail + ".")
