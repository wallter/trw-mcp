...
Three components, each ``active``, ``degraded`` (with the one-line fix) or ``off``:
...
* ``weights`` -- the daemon's recall model is complete in the local Hugging
  Face cache, so the first embed neither downloads nor fails offline.

BM25 has no row: rank-bm25 is a base dependency of trw-memory (PRD-CORE-302 FR08).
...
def daemon_embedding_model() -> str:
    """The encoder the trw-memory daemon loads: ``MEMORY_EMBEDDING_MODEL``, daemon-wide.

    The daemon trw-mcp auto-starts inherits this process's environment, so this
    answers for it. trw-mcp's config has no say in the model.
    """
    from trw_memory.models.config import MemoryConfig

    return MemoryConfig().embedding_model
...
def _weights(model: str) -> RetrievalComponent:
    ...
    return RetrievalComponent("weights", "degraded", f"{model} cache {state.value}", "trw-mcp models fetch")


def probe_retrieval(model: str, *, embeddings_enabled: bool) -> tuple[RetrievalComponent, ...]:
    """The three components, in report order."""
    if not embeddings_enabled:
        ...
        weights = _weights(model)
    return (_vectors(), embeddings, weights)
...
def outside_active_space_note(trw_dir: Path) -> str:
    """The doctor row's count of stored vectors dense recall skips (PRD-CORE-302 FR07).

    An unreachable store or a daemon that has not loaded its model yet says the
    count is unmeasured, never zero.
    """
    ...
    try:
        ...
        return f"; vectors outside the active space: unmeasured ({exc})"
