...
def consolidate_cycle(
    ...
    *,
    ...
    pkg = _pkg()
    _find_clusters = getattr(pkg, "find_clusters")
    _get_config = getattr(pkg, "get_config")
    _LLMClient = getattr(pkg, "LLMClient")
    _summarize_llm = getattr(pkg, "_summarize_cluster_llm")
    _summarize_fallback = getattr(pkg, "_summarize_cluster_fallback")
    _archive = getattr(pkg, "_archive_originals")
