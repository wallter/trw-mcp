...
# The 6 submission-category values accepted by the TRW platform's feedback API
# (PRD-CORE-182; the API's SubmissionCategory enum is canonical). The full-mode
...
def render_feedback_reporting(profile: ClientProfile) -> str:
    ...
    return (
        ...
        f"surface the `{skill_invocation}` skill or call `trw_status(feedback=...)` "
        ...
        f"install issue during your own work, file it via `trw_status(feedback=...)` (or "
        f"`{skill_invocation}`) without waiting for the operator to ask.\n"
        "\n"
