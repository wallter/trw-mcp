"""PRD-CORE-149-FR10: per-family opencode instruction renderers.

Extracted from the monolithic ``_renderer.py`` to keep the orchestrator
``ProtocolRenderer`` class under the 350-LOC ceiling. Each function
returns the literal instruction text historically emitted by the
matching ``ProtocolRenderer._render_opencode_<family>`` method.
"""
