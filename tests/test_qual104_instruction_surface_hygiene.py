...
def _resolve_instruction_text(root: Path, rel: str) -> str:
    """Return an instruction file's text with its ``@``-imports resolved.

    PRD-QUAL-104-FR03 was amended by operator decision (2026-07-28): an
    EAGERLY-RESOLVED ``@``-include satisfies "states the gate verbatim". FR03's
    original rationale was that the instruction file is the protocol carrier "for
    a client that has no other channel into the gate" — an eager include IS such a
    channel, and did not exist when the requirement was written. Both Claude
    Code's ``@path`` and Copilot CLI's ``@relpath`` load at session start, so the
    gate text reaches the model either way.

    What did NOT change: the gate must still be *reachable*. Resolving the import
    is a stricter check than a raw read — a dangling include contributes nothing
    here, so the assertion fails, which a raw substring check could not tell apart
    from success.
    """
    target = root / rel
    ...
    for line in text.splitlines():
        ...
        if stripped.startswith("@") and len(stripped.split()) == 1:
            imported = root / stripped[1:]
            if imported.is_file():
                parts.append(imported.read_text(encoding="utf-8"))
    return "\n".join(parts)
...
class TestLightClientGateInjection:
    ...
    def test_copilot_instructions_gate_present(self, tmp_path: Path) -> None:
        ...
        body = _resolve_instruction_text(tmp_path, ".github/copilot-instructions.md")
        assert _DELIVER_GATE_PHRASE in body
        assert _SESSION_START_PHRASE in body
    ...
    def test_gate_text_survives_advisory_deliver_gate_mode(
        ...
        try:
            ...
            for rel in (
                ...
                body = _resolve_instruction_text(tmp_path, rel)
...
def _write_framework_md(root: Path, version_line: str = "v99.9_TRW — MODEL-AGNOSTIC FRAMEWORK") -> None:
...
class TestVersionDrift:
    ...
    def test_stale_version_flagged_with_correct_version(self, tmp_path: Path) -> None:
        """v25_TRW token while canonical is v99.9 -> version_drift."""
        (tmp_path / "AGENTS.md").write_text("This follows the v25_TRW protocol.\n", encoding="utf-8")
        _write_framework_md(tmp_path, "v99.9_TRW — MODEL-AGNOSTIC FRAMEWORK")
        proc = _run_lint("--strict", cwd=tmp_path)
        assert proc.returncode == 1, proc.stdout
        assert "version_drift" in proc.stdout, proc.stdout
        assert "v25_TRW" in proc.stdout
        assert "v99.9" in proc.stdout

    def test_matching_version_clean(self, tmp_path: Path) -> None:
        """A v99.9_TRW token matching canonical v99.9 is not flagged."""
        (tmp_path / "AGENTS.md").write_text("This follows the v99.9_TRW protocol.\n", encoding="utf-8")
        _write_framework_md(tmp_path, "v99.9_TRW — MODEL-AGNOSTIC FRAMEWORK")
        proc = _run_lint("--strict", cwd=tmp_path)
