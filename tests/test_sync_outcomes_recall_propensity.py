...
def test_only_receipts_count_as_recalls(tmp_path: Path) -> None:
    ...
    _write_recall_tracking(
        ...
        [
            {"learning_id": "L-1", "query": "q1", "outcome": None},
            {"learning_id": "L-1", "query": "q2", "outcome": None, "surface": "before_edit_hint"},
            {"learning_id": "L-1", "query": "q3"},  # absent outcome key: a legacy receipt
            {"learning_id": "L-1", "outcome": "positive", "source": "explicit_feedback"},
            {"learning_id": "L-1", "outcome": "negative", "source": "explicit_feedback"},
            {"learning_id": "L-1", "outcome": "build_passed"},  # unknown value
        ],
    )

    agg = _aggregate_recall_outcomes(trw_dir)
    assert agg["L-1"]["recall_count"] == 3
    assert agg["L-1"]["positive"] == 1
...
def test_sync_payload_after_session_carries_no_session_keys_or_observations(tmp_path: Path) -> None:
    ...
    recall_outcomes = payload["propensity_data"]["recall_outcomes"]  # type: ignore[index]
    assert set(recall_outcomes["L-1"]) == {
        "learning_id",
        "recall_count",
        "positive",
        "negative",
