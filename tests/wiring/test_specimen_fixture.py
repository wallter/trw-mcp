...
SPECIMENS: tuple[tuple[str, str], ...] = (
    ...
    (
        "code_search.py mode='semantic' — reachable, registered, structurally empty",
...
def test_beats_generic_dead_code_scan(live_result: DetectorResult) -> None:
    ...
    # Threshold tracks the list. It was 3; the schema-mirror-parity specimen
    # closed when trw_entity_risk_map was removed (its payload was the one mirror
    # the guard could not cover). Lowering it is honest ONLY because the list
    # shrank for that reason — if this ever drops to 0 the test has stopped
    # earning its place and should be deleted, not weakened further.
    assert len(recalled) >= 2, f"only {len(recalled)} specimens recalled that the generic scan missed"
...
def test_full_scan_under_ten_seconds(live_result: DetectorResult, repo_root: Path) -> None:
    ...
    assert live_result.duration_seconds < 10.0, (
        f"repo-wide scan of {repo_root} took {live_result.duration_seconds:.2f}s, over the 10s budget"
    )
