"""Update, utility, and status-list tests for memory_adapter.

PRD-CORE-280: access-tracking behaviour (``record_surfaced``) is covered in
one place, ``tests/test_record_surfaced.py``, rather than here.
"""
...
class TestUpdateLearning:
    def test_status_change(self, fake_memory_store: FakeMemoryStore, trw_dir: Path) -> None:
    ...
    def test_impact_change(self, fake_memory_store: FakeMemoryStore, trw_dir: Path) -> None:
    ...
    def test_not_found(self, fake_memory_store: FakeMemoryStore, trw_dir: Path) -> None:
        result = update_learning(trw_dir, "L-nonexistent")
        assert result["status"] == "not_found"

    def test_invalid_status(self, fake_memory_store: FakeMemoryStore, trw_dir: Path) -> None:
        store_learning(trw_dir, "L-u3", "s", "d")
        result = update_learning(trw_dir, "L-u3", status="bad")
        assert result["status"] == "invalid"

    def test_no_changes(self, fake_memory_store: FakeMemoryStore, trw_dir: Path) -> None:
        store_learning(trw_dir, "L-u4", "s", "d")
        result = update_learning(trw_dir, "L-u4")
        assert result["status"] == "no_changes"

    def test_return_shape_keys(self, fake_memory_store: FakeMemoryStore, trw_dir: Path) -> None:
    ...
    def test_allows_obsolete_poisoned_status(self, fake_memory_store: FakeMemoryStore, trw_dir: Path) -> None:
...
class TestUtilities:
    def test_count_entries(self, fake_memory_store: FakeMemoryStore, trw_dir: Path) -> None:
        assert count_entries(trw_dir) == 0
        store_learning(trw_dir, "L-cnt1", "s", "d")
        assert count_entries(trw_dir) == 1

    def test_list_active_learnings(self, fake_memory_store: FakeMemoryStore, trw_dir: Path) -> None:
    ...
    def test_find_entry_by_id_found(self, fake_memory_store: FakeMemoryStore, trw_dir: Path) -> None:
    ...
    def test_find_entry_by_id_not_found(self, fake_memory_store: FakeMemoryStore, trw_dir: Path) -> None:
...
class TestListEntriesByStatus:
    ...
    def test_active_filter(self, fake_memory_store: FakeMemoryStore, trw_dir: Path) -> None:
    ...
    def test_min_impact_filter(self, fake_memory_store: FakeMemoryStore, trw_dir: Path) -> None:
    ...
    def test_invalid_status_returns_empty(self, fake_memory_store: FakeMemoryStore, trw_dir: Path) -> None:
        """Invalid status string returns empty list."""
        result = list_entries_by_status(trw_dir, status="bogus_status")
        assert result == []

    def test_dict_fields_present(self, fake_memory_store: FakeMemoryStore, trw_dir: Path) -> None:
    ...
    def test_resolved_status(self, fake_memory_store: FakeMemoryStore, trw_dir: Path) -> None:
