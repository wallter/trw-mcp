...
async def test_review_visible_on_kernel_only_surface(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    resolves a 12-tool surface that contains ``trw_review``.

    12 = the 9 KERNEL_TOOLS + trw_build_check + trw_init (bootstrap) + trw_review.
    """
    ...
    assert names == set(KERNEL_TOOLS) | {"trw_build_check", "trw_init", "trw_review"}
    assert len(names) == 12
