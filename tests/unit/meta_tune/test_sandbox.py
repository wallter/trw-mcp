...
def test_sandbox_determinism_under_fixed_seed(tmp_path: Path) -> None:
    ...
    r1 = run_sandboxed(cmd, timeout_s=5.0)
    r2 = run_sandboxed(cmd, timeout_s=5.0)
...
def test_sandbox_degraded_mode_logs_warning_on_non_linux(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    ctx = ProbeIsolationContext(timeout_s=0.1, memory_cap_mb=64)
    with ctx as runner:
...
def test_sandbox_context_manager_cleanup() -> None:
    ...
    ctx = ProbeIsolationContext(timeout_s=1.0, memory_cap_mb=64)
    with ctx as runner:
...
def test_sandbox_allow_network_skips_unshare() -> None:
    ...
    with ProbeIsolationContext(timeout_s=1.0, allow_network=True) as runner:
...
def test_sandbox_rss_darwin_branch(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    r = run_sandboxed([sys.executable, "-c", "pass"], timeout_s=5.0)
