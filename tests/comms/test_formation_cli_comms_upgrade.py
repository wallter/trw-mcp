...
parser routes to it, that it is orchestrator-only, that every refusal exits non-zero with
its reason, and that the real console entry point (``trw_mcp.server:main``) reaches the
actual migration.
"""
...
def test_refusals_exit_nonzero_with_their_reason(
    ...
    manifest = _v3_mailbox(formation_env)
    member_run = str(formation_env.member_runs["impl-1"])
    with pytest.raises(SystemExit) as not_orchestrator:
        run_formation(_parse("comms-upgrade", "--run", member_run))
