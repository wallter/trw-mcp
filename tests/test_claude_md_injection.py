...
class TestMergePreservesUserContent:
    ...
    def test_user_content_preserved(self, tmp_path: Path) -> None:
        ...
        from trw_mcp.state.claude_md._write_guard import non_generated_bytes

        markers = (TRW_MARKER_START, TRW_MARKER_END)
        before_non_generated = non_generated_bytes(user_content, markers)

        merge_trw_section(target, new_section, max_lines=500, project_root=tmp_path)
        ...
        # PRD-FIX-123-FR02: not merely "the strings are still there" — the
        # measured non-generated byte count must not have shrunk.
        assert non_generated_bytes(result, markers) == before_non_generated
...
class TestMergeReplacesTrwSection:
    ...
    def test_full_replacement(self, tmp_path: Path) -> None:
        ...
        markers = (TRW_MARKER_START, TRW_MARKER_END)
        before_non_generated = non_generated_bytes(original, markers)

        merge_trw_section(target, new_section, max_lines=500, project_root=tmp_path)

        result = target.read_text(encoding="utf-8")
        # PRD-FIX-123-FR02: everything outside the markers survives byte for byte.
        assert non_generated_bytes(result, markers) == before_non_generated
