...
def test_trw_code_index_update_returns_stats_and_manifest_path_without_file_content(tmp_path: Path) -> None:
    ...
    result = build_code_index(repo_root=str(tmp_path), force=False, paths=None)
...
def test_trw_code_index_update_supports_force_and_path_limits(tmp_path: Path) -> None:
    ...
    first = build_code_index(repo_root=str(tmp_path), force=False, paths=None)

    (tmp_path / "a.py").write_text("changed\n", encoding="utf-8")
    limited = build_code_index(repo_root=str(tmp_path), force=False, paths=["a.py"])
    forced = build_code_index(repo_root=str(tmp_path), force=True, paths=["a.py"])
...
def test_trw_code_index_update_uses_configured_discovery_limits(
    ...
    monkeypatch.setattr(
        ...
        lambda: SimpleNamespace(
            ...
            code_index_bounds=CodeIndexBounds(),
        ),
    )
...
def test_trw_code_index_update_returns_structured_failure_for_filesystem_errors(tmp_path: Path) -> None:
    ...
    result = build_code_index(repo_root=str(repo_root), force=False, paths=None)
