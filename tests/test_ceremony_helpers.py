...
class TestPerformSessionRecalls:
    ...
    def test_wildcard_recall_returns_learnings(
        self, trw_dir: Path, config: TRWConfig, reader: FileStateReader,
    ) -> None:
        mock_entries = [
    ...
    def test_focused_recall_deduplicates(
        ...
        with (
            ...
            learnings, _, extra = perform_session_recalls(
                trw_dir, "test query", config, reader,
            )
...
class TestPhaseContextualRecall:
    ...
    def test_includes_phase_tags_from_run_status(
        self, trw_dir: Path, config: TRWConfig, run_dir: Path,
    ) -> None:
        mock_entries = [
        ...
        with patch(
            ...
            result = _phase_contextual_recall(
                trw_dir, "", config, run_dir, run_status,
            )
    ...
    def test_ranks_and_caps_results(
        ...
        mock_entries = [
            {"id": f"L-{i:03d}", "summary": f"Entry {i}", "impact": 0.5 + i * 0.01}
            for i in range(20)
        ]
...
class TestCheckDeliveryGates:
    ...
    def test_review_warning_on_critical_findings(
        self, run_dir: Path, reader: FileStateReader, writer: FileStateWriter,
    ) -> None:
        review_path = run_dir / "meta" / "review.yaml"
        writer.write_yaml(review_path, {
            "verdict": "block",
            "critical_count": 3,
        })
...
class TestFinalizeRun:
    ...
    def test_returns_empty_dict(
        self, run_dir: Path, trw_dir: Path, config: TRWConfig,
