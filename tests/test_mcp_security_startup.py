...
def test_init_security_threads_shadow_config_and_auto_release() -> None:
    middleware = init_security(
        MCPSecurityConfig(
            anomaly={"mode": "shadow", "sigma_threshold": 3.0, "window_seconds": 30},
            quarantine={"auto_release": True},
        )
    )

    assert middleware._detector._config.mode == "shadow"
...
def test_init_security_rejects_enforce_without_validated_baseline() -> None:
    with pytest.raises(MCPSecurityConfigError, match="validated production rate-baseline loader") as exc_info:
        init_security(MCPSecurityConfig(anomaly={"mode": "enforce"}))

    assert "mode to 'shadow'" in str(exc_info.value)


def test_build_middleware_propagates_anomaly_enforce_config_error() -> None:
    reload_config(TRWConfig(meta_tune={"enabled": False}, security={"mcp": {"anomaly": {"mode": "enforce"}}}))

    with pytest.raises(MCPSecurityConfigError, match="validated production rate-baseline loader"):
        from trw_mcp.server._app import _build_middleware
