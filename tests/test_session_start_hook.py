...
def test_delegation_guidance_preserves_required_independent_review(tmp_path: Path, source_hook: Path) -> None:
    ...
    for source in ("startup", "clear"):
        project_root, local_hook = _copy_hook_to_temp(tmp_path / source, source_hook)
        output = _run_hook(local_hook, project_root, source)
        assert "not to verify your own work" not in output
        assert "Required independent review is separate from routine self-checks" in output

    from trw_mcp.state.claude_md.sections._tool_lifecycle import load_tool_lifecycle

    body = load_tool_lifecycle()
    assert "not to verify your own work" not in body
    assert "Required independent review is separate from routine self-checks" in body
...
def test_protocol_still_emitted_when_no_instruction_file_carries_it(tmp_path: Path, hook_path: Path) -> None:
    ...
    for source in ("resume", "compact", "clear"):
        project_root, local_hook = _copy_hook_to_temp(tmp_path / hook_path.parent.name / f"bare-{source}", hook_path)
        (project_root / ".trw" / "context" / "behavioral_protocol.md").write_text(
            "# TRW Behavioral Protocol\n\n### Tool Lifecycle\n\nUNIQUE-PROTOCOL-BODY-MARKER\n",
            encoding="utf-8",
        )
...
@pytest.mark.parametrize("hook_path", _HOOK_CASES)
def test_mid_session_emissions_stay_within_byte_budget(tmp_path: Path, hook_path: Path) -> None:
...
@pytest.mark.parametrize(
    "ups_hook",
    [
        pytest.param(path, marks=() if path.is_relative_to(PACKAGE_ROOT) else requires_monorepo)
