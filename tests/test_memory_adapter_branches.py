...
class TestGetBackendAutoResolve:
    def test_auto_resolve_trw_dir_when_none(self, trw_dir: Path) -> None:
        ...
        with patch(
            "trw_mcp.state._paths.resolve_trw_dir", return_value=trw_dir
        ):
...
class TestKeywordSearchMultiToken:
    ...
    def test_multi_token_no_common(self, trw_dir: Path) -> None:
        ...
        backend = get_backend(trw_dir)
        backend.store(
            MemoryEntry(id="L-nc1", content="alpha", detail="only alpha")
        )
        backend.store(
            MemoryEntry(id="L-nc2", content="beta", detail="only beta")
        )
...
class TestSearchEntriesHybrid:
    def test_embedder_none_falls_back_to_keyword(self, trw_dir: Path) -> None:
        ...
        backend = get_backend(trw_dir)
        backend.store(
            MemoryEntry(id="L-fb1", content="fallback test", detail="d")
        )
        with patch(
            "trw_mcp.state.memory_adapter.get_embedder", return_value=None
        ):
    ...
    def test_embed_query_returns_none(self, trw_dir: Path) -> None:
        ...
        backend = get_backend(trw_dir)
        backend.store(
            MemoryEntry(id="L-en1", content="embed none", detail="d")
        )
        mock_embedder = MagicMock()
        mock_embedder.embed.return_value = None
        with patch(
    ...
    def test_empty_vector_hits(self, trw_dir: Path) -> None:
        ...
        backend = get_backend(trw_dir)
        backend.store(
            MemoryEntry(id="L-ev1", content="vector empty", detail="d")
        )
        mock_embedder = MagicMock()
    ...
    def test_hybrid_rrf_fusion_success(self, trw_dir: Path) -> None:
        ...
        with (
            ...
            patch(
                "trw_memory.retrieval.fusion.rrf_fuse", mock_fuse
            ),
        ):
    ...
    def test_hybrid_vector_only_entry_fetched(self, trw_dir: Path) -> None:
        ...
        entry2 = MemoryEntry(
            id="L-vo2", content="only in vectors", detail="d2", importance=0.9
        )
        backend.store(entry1)
        backend.store(entry2)
        ...
        def limited_keyword(
            be: Any, query: str, **kwargs: Any
        ) -> list[MemoryEntry]:
            return [entry1]

        mock_fuse = MagicMock(
            return_value=[("L-vo1", 1.0), ("L-vo2", 0.8)]
        )
        ...
        with (
            ...
            patch.object(backend, "search_vectors", return_value=vector_hits),
            patch(
                "trw_mcp.state.memory_adapter._keyword_search", limited_keyword
            ),
    ...
    def test_hybrid_vector_entry_filtered_by_min_impact(
        self, trw_dir: Path
    ) -> None:
        """Vector-only entry below min_impact is filtered (line 434)."""
        backend = get_backend(trw_dir)
        entry1 = MemoryEntry(
            id="L-fi1", content="keyword hit", detail="d1", importance=0.9
        )
        low_impact_entry = MemoryEntry(
            id="L-fi2", content="low impact vector", detail="d2", importance=0.1
        )
        backend.store(entry1)
        ...
        mock_fuse = MagicMock(
            return_value=[("L-fi1", 1.0), ("L-fi2", 0.8)]
        )
        ...
        with (
            ...
            patch.object(backend, "search_vectors", return_value=vector_hits),
            patch(
                "trw_mcp.state.memory_adapter._keyword_search", limited_kw
            ),
            patch("trw_memory.retrieval.fusion.rrf_fuse", mock_fuse),
        ):
            results = _search_entries(
                backend, "keyword", min_impact=0.5
            )
            ids = [e.id for e in results]
            assert "L-fi2" not in ids

    def test_hybrid_vector_entry_filtered_by_status(
        self, trw_dir: Path
    ) -> None:
        ...
        mock_fuse = MagicMock(
            return_value=[("L-fs1", 1.0), ("L-fs2", 0.8)]
        )
        ...
        with (
            ...
            results = _search_entries(
                backend, "kw", mem_status=MemoryStatus.ACTIVE
            )
            ids = [e.id for e in results]
            assert "L-fs2" not in ids

    def test_hybrid_vector_entry_filtered_by_tags(
        self, trw_dir: Path
    ) -> None:
        """Vector-only entry with missing tags is filtered (lines 438-439)."""
        backend = get_backend(trw_dir)
        entry1 = MemoryEntry(
            id="L-ft1", content="kw hit", detail="d1", tags=["python"]
        )
        wrong_tags = MemoryEntry(
            id="L-ft2", content="wrong tags", detail="d2", tags=["rust"]
        )
        backend.store(entry1)
        ...
        mock_fuse = MagicMock(
            return_value=[("L-ft1", 1.0), ("L-ft2", 0.8)]
        )
    ...
    def test_hybrid_exception_falls_back(self, trw_dir: Path) -> None:
        ...
        backend = get_backend(trw_dir)
        backend.store(
            MemoryEntry(id="L-ex1", content="exception fallback", detail="d")
        )
...
class TestFindYamlPathIndexSkip:
    ...
    def test_partial_match_with_index_yaml_present(self, trw_dir: Path) -> None:
        ...
        (entries_dir / "index.yaml").write_text("index: true\n")
        (entries_dir / "2026-01-01-L-idx001-some-summary.yaml").write_text(
            "id: L-idx001\n"
        )
...
class TestBackfillEmbeddings:
    ...
    def test_skips_empty_content(self, trw_dir: Path) -> None:
        ...
        backend.store(
            MemoryEntry(id="L-sk1", content="", detail="")
        )
