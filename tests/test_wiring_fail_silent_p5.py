...
PRD-CORE-280 slice e1: ``test_w12_rejected_items_are_counted_and_reported``
exercised ``SyncPuller.merge_team_learnings`` by constructing a real
``SQLiteBackend`` and patching ``memory_adapter.get_backend`` -- a patch that
no longer does anything, since ``merge_team_learnings`` already reads and
writes through ``_store_selection.selected_store`` (per its own docstring).
Ported to ``fake_memory_store``: the per-item accounting (inserted/
skipped_no_id/invalid/rejected) is store-agnostic bookkeeping the fake's
``find_synced``/``apply_synced`` exercise the same way. W06's graph probe reads
the store's ``health`` block, so it runs on the fake too; the rest construct
their own throwaway sqlite fixtures unrelated to a checkout's store or patch
production seams directly.
"""
...
def test_w06_pipeline_probe_reports_not_measured(
    ...
    That the store never answers "no relations" for a namespace it could not
    read is trw-memory's ``test_tools_status_health``.
    """
    ...
    def _refused(namespace: str) -> None:
        raise ValueError(f"memory_status measured no health for {namespace}: storage error")

    monkeypatch.setattr(fake_memory_store, "health", _refused)
...
def test_w11_update_learning_reports_lookup_unavailable(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    from trw_mcp.exceptions import NamespaceEnumerationError
    from trw_mcp.state import _memory_update, _store_selection

    class _UnsearchableStore:
        def correct(self, _learning_id: str, _patch: object) -> object:
            raise NamespaceEnumerationError("could not enumerate namespaces")

    monkeypatch.setattr(_store_selection, "selected_store", lambda _dir: (_UnsearchableStore(), "default"))
...
def test_w11_graph_related_reports_lookup_unavailable(fake_memory_store: FakeMemoryStore) -> None:
...
def test_w12_rejected_items_are_counted_and_reported(tmp_path: Path, fake_memory_store: object) -> None:
    ...
    PRD-CORE-280 slice e1: ``merge_team_learnings`` reads/writes through
    ``selected_store`` (see module docstring), so this routes through
    ``fake_memory_store`` rather than a hand-built ``SQLiteBackend``.
    """
