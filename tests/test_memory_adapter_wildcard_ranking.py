...
The wildcard branch of ``recall_learnings`` fetches entries via the store's
``recall()``; for the fake that means ``FakeMemoryStore``'s ``"default"``
namespace, walked in whatever order the rows dict yields, so nothing here
enforces recency ordering on its own — exactly like the pre-fix real backend,
which ordered by ``updated_at DESC`` only. These tests seed the rows directly
(bypassing ``store.put``'s auto timestamp) to reproduce the recency-ordered
input the ranking has to overturn, then assert the RETURNED ORDER is
impact-driven.

Each test FAILS against the pre-fix code (which returned store order
unchanged): the high-impact entry is deliberately the OLDEST, so a
recency-only path puts it last.
"""
...
def _seeded_recall(
    fake_memory_store: FakeMemoryStore, trw_dir: Path, entries: list[MemoryEntry], **kwargs: object
) -> list[dict[str, object]]:
    for entry in entries:
        fake_memory_store.rows[("default", entry.id)] = entry
    return recall_learnings(trw_dir, "*", status="active", **kwargs)  # type: ignore[arg-type]


class TestWildcardRanksByImpact:
    def test_wildcard_returns_highest_impact_first_not_newest(
        self, fake_memory_store: FakeMemoryStore, trw_dir: Path
    ) -> None:
        results = _seeded_recall(
            fake_memory_store, trw_dir, _recency_ordered_entries(), min_impact=0.0, max_results=10, compact=True
        )
    ...
    def test_wildcard_top_result_is_high_impact_entry(self, fake_memory_store: FakeMemoryStore, trw_dir: Path) -> None:
        results = _seeded_recall(
            fake_memory_store, trw_dir, _recency_ordered_entries(), min_impact=0.0, max_results=10, compact=True
        )
    ...
    def test_precondition_backend_returns_recency_order(self) -> None:
    ...
    def test_wildcard_ranking_failure_falls_open_to_recency(
        self, fake_memory_store: FakeMemoryStore, trw_dir: Path
    ) -> None:
